...
* N.B. CRAMs may be substituted interchangeably with BAMs.  For technical reasons please name these as BAMs (the symlink, not the file itself).  For example, if you have a CRAM, `/a/b/sample.cram`, we want a symlink in `input/bams`, and run `ln -s /a/b/sample.cram input/bams/sample.bam`.  This leaves the original file as a CRAM (appropriately) and just tells the pipeline to treat it as a BAM.
